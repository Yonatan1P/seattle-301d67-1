...
function displayPics(){
  ...
  //the console.log(rando) could not be performed and threw an error into the code because it 
  //is put into block scope by the let call. The var keyword puts the rando variable into function
  //scope, but also creates a property on the global object which is being called by console.log
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(const i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${allProducts[i].name} has ${allProducts[i].votes} votes in ${allProducts[i].views} views`);
    }
  }
